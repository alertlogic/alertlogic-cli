...
class ListScanQueues(alertlogiccli.command.Command):
    ...
    def configure_parser(self, subparsers):
        parser = subparsers.add_parser("list_scan_queues", help="lists hosts in scan queues for a given deployment")
    ...
    def execute(self, context):
        ...
        try:
            response = scan_scheduler.listscanassets(account_id=args["account_id"],
...
class ScanHost(alertlogiccli.command.Command):
    ...
    def configure_parser(self, subparsers):
        ...
        parser.add_argument("--host_key", dest="asset_key", required=True, help="a host key to put in the queue")

    def execute(self, context):
        args = context.get_final_args()
        scan_scheduler = context.get_services().scan_scheduler
        try:
            response = scan_scheduler.scanasset(account_id=args["account_id"],
                                                environment_id=args["deployment_id"],
